...
import pandas as pd
from pipeline_penguin.data_premise.sql import DataPremiseSQLCheckInArray
    ...
    def mock_data_node():
        class MockDataNode:
            ...
            def get_connector(self, *args, **kwargs):
                connector_mock = MagicMock()
                connector_mock.run = MagicMock(
                    return_value=pd.DataFrame([3, 3, 2, 1, 2], columns=["result"])
                )
                return connector_mock
...
class TestDataPremiseSQLCheckIsNull:
    def test_instance_type(self, _mock_data_node_with_passed_validation):
        ...
        data_premise = DataPremiseSQLCheckInArray(
            "test_name", data_node, "test_column", []
        )
        assert isinstance(data_premise, DataPremiseSQLCheckInArray)
